...
const UploadIdMobile = () => {
  ...
  console.log(query.get("token"), query.get("userId"), "console");

  localStorage.setItem("journeyToken", query.get("token"));
  localStorage.setItem("userIdM", query.get("userId"));

  useEffect(() => {
    toast("Loading Journey Interface");
    setTimeout(() => {
      window.location.replace(window.location.origin + "/idscan.html");
    }, 3000);
  }, []);
  return (
    <OneLayout showNotif={false}>
      <Centeredbox>
        {/* carousel */}
        <UploadIdBox>
          ...
          <br />
          <center>
            <h2>Loading Journey Interface...</h2>
          </center>
        </UploadIdBox>
      </Centeredbox>
    </OneLayout>
